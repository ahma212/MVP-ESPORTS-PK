...
// IMPORTANT:
// The roster reconciliation above can change player/team state in Supabase
// (including alive/dead state and newly added players). Re-read the roster
// after reconciliation so the UI never shows stale team/player counters.

const { data: refreshedTeams, error: refreshedTeamsError } = await supabase
  .from('live_broadcast_teams')
  .select('*')
  .eq('session_id', sessionId)
  .order('rank', { ascending: true, nullsFirst: false });

if (refreshedTeamsError) throw refreshedTeamsError;
...
const freshTypedPlayers = (refreshedPlayers || []) as BroadcastPlayerRow[];

// Re-apply Team/Player Number mapping from the real slot-booking roster.
let freshEnrichedPlayers = freshTypedPlayers;

if (rosterSourceMatchId && supabase) {
  const { data: freshRosterBookings, error: freshRosterError } = await supabase
    .from('slot_bookings')
    .select('player_id,user_id,player_uid,player_ign,team_name,slot_number')
    .eq('match_id', rosterSourceMatchId)
    .eq('status', 'confirmed')
  ...
  if (!freshRosterError && freshRosterBookings) {
    const freshRosterMap = freshRosterBookings.map((booking: any, index: number) => ({
      booking,
    ...
    const freshMappedPlayers = freshTypedPlayers.map((player) => {
      const matches = freshRosterMap.filter(({ booking }) =>
    ...
    const freshCanonicalPlayers = new Map<string, BroadcastPlayerRow>();

    for (const player of freshMappedPlayers) {
      const key = rosterPlayerIdentityKey(player);
      const existingPlayer = freshCanonicalPlayers.get(key);

      if (
        !existingPlayer ||
        playerStatePriority(player) > playerStatePriority(existingPlayer)
      ) {
        freshCanonicalPlayers.set(key, player);
      }
    }

    freshEnrichedPlayers = Array.from(freshCanonicalPlayers.values())
      .sort(
        (a, b) =>
          Number(a.slot_number || 0) - Number(b.slot_number || 0)
      );
  }
}

setTeams((refreshedTeams || []) as BroadcastTeamRow[]);
setPlayers(freshEnrichedPlayers);
