...
export const MyMatchesView: React.FC<MyMatchesViewProps> = ({
  ...
  // Filter booked matches by status tab
    ...
    <div className="space-y-4 pb-20 animate-in fade-in duration-200">
      ...
      {isLoading && bookedMatchesList.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 space-y-0">
          {filteredMatches.map(({ match, booking }) => {
            ...
            // Lookup Result for completed matches
            const matchResult = allMatchResults.find((r) => String(r.match_id) === String(match.id));
            const playerStats = matchResult?.results?.find(
              (p) =>
                p.player_ign.toLowerCase() === booking.player_ign.toLowerCase() ||
                (booking.team_name && p.player_ign.toLowerCase() === booking.team_name.toLowerCase())
            );

            return (
              <div
                key={booking.id || `${match.id}-${booking.slot_number}`}
              ...
              >
                ...
                <div className="flex justify-between items-center text-xs">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-black text-[#00e5ff] uppercase tracking-wider bg-[#00e5ff]/15 px-2 py-0.5 rounded border border-[#00e5ff]/40">
                      SLOT #{booking.slot_number} CONFIRMED
                    </span>
                ...
                {/* Player details */}
                <div className="grid grid-cols-2 gap-2 text-xs p-2.5 rounded-xl bg-[#020710] border border-gray-800">
                  <div>
                    <span className="text-[9px] text-gray-400 block font-semibold uppercase">REGISTERED IGN</span>
                    <span className="font-bold text-white truncate block">{booking.player_ign}</span>
                  </div>
                  <div>
                    <span className="text-[9px] text-gray-400 block font-semibold uppercase">PUBG CHARACTER UID</span>
                    <span className="font-bold text-[#00e5ff] truncate block">{booking.player_uid || 'Registered'}</span>
                  </div>
                </div>
                      ...
                      {mapsList.map((mapName, idx) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {isPublished ? (
                              <div className="flex justify-between items-center mt-2 p-2 rounded-lg bg-black/60 border border-emerald-500/40">
                                <div>
                                  <span className="text-[8px] text-emerald-400 font-black uppercase block">
                                    ROOM ID & PASSWORD
                                  </span>
                                  <span className="text-xs font-black text-white font-mono">
                                    {roomId} &bull; {roomPass || '123'}
                                  </span>
                                </div>
                                <button
                                  type="button"
                                  onClick={() => handleCopy(`${roomId} | ${roomPass}`, `${match.id}-${idx}`)}
                                  className="px-2.5 py-1 rounded bg-emerald-500/20 text-emerald-300 font-bold text-[10px] flex items-center gap-1 border border-emerald-500/40 hover:bg-emerald-500/30 active:scale-95 transition-all"
                                >
                                  {copiedIdMap[`${match.id}-${idx}`] ? (
                                    <>
