...
export const AdminPanelModal: React.FC<AdminPanelModalProps> = ({
  ...
  // Sync Room ID/Pass ONLY when match or map selection CHANGES
  useEffect(() => {
    const activeMatch = (matches || []).find((m) => m.id === selectedMatchId) || (matches || [])[0];
    if (activeMatch) {
      if (activeMatch.type === 'tournament') {
        const mapsList = activeMatch.maps && activeMatch.maps.length > 0 
          ? activeMatch.maps 
          : ['Erangel', 'Miramar', 'Rondo'];
        
        const rids = Array(mapsList.length).fill('');
        const rpasses = Array(mapsList.length).fill('');
        const rtimers = Array(mapsList.length).fill(0);

        mapsList.forEach((_, idx) => {
          const cred = activeMatch.room_credentials?.[idx];
          if (cred) {
            rids[idx] = cred.room_id || '';
            rpasses[idx] = cred.room_password || '';
            rtimers[idx] = cred.release_timer_minutes || 0;
          } else if (idx === 0 && activeMatch.room_id) {
            rids[idx] = activeMatch.room_id;
            rpasses[idx] = activeMatch.room_password || '';
            rtimers[idx] = 0;
          }
        });
        setMultiRoomIds(rids);
        setMultiRoomPasses(rpasses);
        setMultiRoomTimers(rtimers);
      } else {
        const cred = activeMatch.room_credentials?.[selectedMapIndex];
        if (cred) {
          setRoomId(cred.room_id || '');
          setRoomPass(cred.room_password || '');
          setReleaseTimerMinutes(cred.release_timer_minutes || 0);
        } else if (selectedMapIndex === 0 && activeMatch.room_id) {
          setRoomId(activeMatch.room_id);
          setRoomPass(activeMatch.room_password || '');
          setReleaseTimerMinutes(0);
        } else {
          setRoomId('');
          setRoomPass('');
          setReleaseTimerMinutes(0);
  ...
  }, [selectedMatchId, selectedMapIndex]);

  const renderThumbnailSelector = (isEdit: boolean) => {
    const currentType = type;
    
    if (currentType !== 'tournament') {
      return (
        <div className="space-y-2 p-3 bg-[#030a16]/60 border border-gray-800 rounded-xl">
          <label className="text-[11px] font-black text-gray-300 block uppercase tracking-wider">
            Match Thumbnail (Header Image)
          </label>
          
          <div className="flex flex-col sm:flex-row gap-3 items-start">
            {/* Gallery File Upload Button */}
            <div className="w-full sm:w-auto">
              <label className={`cursor-pointer py-2 px-3.5 rounded-lg text-xs flex items-center justify-center gap-2 transition-all ${
                isUploadingBanner
                  ? 'bg-gray-800 text-gray-400 cursor-not-allowed border border-gray-700'
                  : (isAnyUploading
                      ? 'bg-gray-800/40 text-gray-500 cursor-not-allowed border-gray-800'
                      : 'bg-[#00e5ff]/10 hover:bg-[#00e5ff]/20 text-[#00e5ff] font-bold border border-[#00e5ff]/30')
              }`}>
                <span>{isUploadingBanner ? '⏳ Uploading to Supabase...' : '📁 Upload Thumbnail (Device/Gallery)'}</span>
                <input
                  type="file"
                  accept="image/*"
              ...
              <span className="text-[9px] text-gray-500 mt-1 block font-medium">Supports JPG, PNG up to 5MB (Uploaded directly to Supabase Storage)</span>
            </div>
            ...
            <div className="flex-1 w-full">
              <input
                ...
                className="w-full p-2 rounded bg-[#030a16] border border-gray-700 text-white text-xs placeholder:text-gray-600 focus:border-[#00e5ff] outline-none"
              />
            </div>
          </div>
          ...
          {bannerUrl && (
            <div className="relative mt-2 rounded-lg overflow-hidden border border-[#00e5ff]/30 bg-black/40 h-28 flex items-center justify-center">
              <img
                src={bannerUrl}
                alt="Banner Preview"
              ...
              <button
                ...
                className="absolute top-2 right-2 p-1 rounded-full bg-red-600 hover:bg-red-500 text-white shadow-lg transition-all"
                title="Remove Custom Image"
              >
                <X className="w-3.5 h-3.5" />
              </button>
              <div className="absolute bottom-2 left-2 px-2 py-0.5 rounded bg-black/60 text-[#00e5ff] text-[9px] font-bold uppercase tracking-widest border border-[#00e5ff]/20">
      ...
      // Grand Tournament Multi-Match dynamic configuration
      const activeTournamentMaps = [...tournamentMaps].slice(0, tournamentMatchCount);
      return (
        <div className="space-y-3.5 p-3.5 bg-[#030a16]/60 border border-gray-800 rounded-xl">
          <div>
            <label className="text-xs font-black text-[#00e5ff] block uppercase tracking-wider">
              🏆 Grand Tournament Map Configurations ({tournamentMatchCount} Matches)
            </label>
            <span className="text-[10px] text-gray-400 block mt-0.5">
              Configure each slot match map, slots, and custom image thumbnail for this tournament series.
            </span>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
            {activeTournamentMaps.map((mapName, idx) => {
              const currentBanner = mapBanners[idx] || '';
              return (
                <div key={idx} className="p-3 rounded-xl bg-[#020710]/80 border border-gray-800 space-y-2.5 flex flex-col justify-between">
